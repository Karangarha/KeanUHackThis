...
import Planets from "../backgrounds/Planets";
...
const ThemeSection = () => {
  ...
  const orbitSvgRef = React.useRef(null);
  const pathFullRef = React.useRef(null);
  const pathTrailRef = React.useRef(null);
  const rocketRef = React.useRef(null);
  const exhaustRef = React.useRef(null);
  const secLabelRef = React.useRef(null);
  const secTextRef = React.useRef(null);
  const nebulaRef = React.useRef(null);
  const stickyRef = React.useRef(null);
  const spaceSectionRef = React.useRef(null);
  const atmoRef = React.useRef(null);
  const cloudsRef = React.useRef(null);
  const scheduleVisibleRef = React.useRef(null);
  const jumpedRef = React.useRef(false);
  const lenis = useLenis();
  const transitionProgress = useMotionValue(0);

  const panelRefs = {
    mercury: React.useRef(null),
    venus: React.useRef(null),
    earth: React.useRef(null),
    mars: React.useRef(null),
  };
  ...
  const EVENTS = useMemo(
    () => ({
      ...
      venus: { arrive: 0.29, panelIn: 0.3, panelOut: 0.4 },
      earth: { arrive: 0.47, panelIn: 0.48, panelOut: 0.56 },
      mars: { arrive: 0.61, panelIn: 0.62, panelOut: 0.7 },
    }),
    [],
  );
  ...
  useEffect(() => {
    ...
    if (!document.getElementById(styleId)) {
      ...
      style.textContent = `
        ...
        .theme-root{background:var(--deep);color:#fff;font-family:var(--font-b)}
        ...
        #space-section-react{position:relative;height:800vh}
        .theme-sticky{position:sticky;top:0;height:100vh;width:100%;overflow:hidden;background:var(--deep)}
        ...
        .pl-mercury{left:17%;top:42%;animation:flt1 6.2s ease-in-out infinite}
        .pl-venus{left:37%;top:57%;animation:flt2 7.4s ease-in-out infinite 1.1s}
        .pl-earth{left:58%;top:40%;animation:flt3 8.1s ease-in-out infinite .5s}
        .pl-mars{left:83%;top:52%;animation:flt4 6.6s ease-in-out infinite 2.2s}
        ...
        .track-panel{position:absolute;z-index:200;width:270px;background:rgba(4,12,38,.78);backdrop-filter:blur(22px);-webkit-backdrop-filter:blur(22px);border:1px solid rgba(0,180,255,.18);border-radius:14px;padding:1.4rem;opacity:0;transform:translateY(18px) scale(.96);transition:opacity .55s cubic-bezier(.22,1,.36,1),transform .55s cubic-bezier(.22,1,.36,1);pointer-events:none;box-shadow:0 4px 40px rgba(0,80,200,.12),inset 0 1px 0 rgba(255,255,255,.05)}
        ...
        .theme-atmo{position:absolute;inset:0;z-index:110;background:linear-gradient(115deg,#62cff4,#2c67f2);opacity:0;pointer-events:none}
        .theme-clouds{position:absolute;inset:0;z-index:120;opacity:0;pointer-events:none;overflow:visible}
        ...
        .cl15{width:40vw;height:120px;left:-10%;top:20%;filter:blur(8px)}.cl16{width:45vw;height:130px;right:-10%;top:40%;filter:blur(10px)}
        .cl17{width:50vw;height:140px;left:10%;top:60%;filter:blur(12px)}.cl18{width:55vw;height:150px;right:5%;top:80%;filter:blur(14px)}
        .cl19{width:60vw;height:160px;left:30%;top:-5%;filter:blur(16px)}
  ...
  useEffect(() => {
    ...
    function buildOrbitPaths() {
      if (!pathFullRef.current || !pathTrailRef.current || !orbitSvgRef.current)
        return;
      const vw = window.innerWidth;
      const vh = window.innerHeight;
    ...
    function updateTrailPath(progress) {
      ...
      for (let i = 1; i <= end; i += 1)
        d += ` L ${orbitPoints[i].x} ${orbitPoints[i].y}`;
    ...
    function panelAlpha(progress, pIn, pOut) {
      ...
      if (progress < pOut)
        return 1 - smooth(invLerp(pOut - FO, pOut, progress));
      return 0;
    }
    ...
    function updateScene(progress) {
      try {
        if (!spaceSectionRef.current) return;

        if (secLabelRef.current) {
          const labelAlpha = Math.max(
            0,
            1 - smooth(invLerp(0, 0.08, progress)),
          );
        ...
        if (secTextRef.current) {
          ...
          const textOut = 1 - smooth(invLerp(0.65, 0.7, progress));
        ...
        if (progress >= ZOOM_START) {
          ...
          const baseSize = isMobile ? 150 : 225;
          const maxS =
            (Math.max(window.innerWidth, window.innerHeight) / baseSize) * 2;
          const scale = 1 + (maxS - 1) * zp;
          const startX = isMobile ? 62 : 58;
          const startY = isMobile ? 38 : 40;
          const currentX = startX + (50 - startX) * zp;
          const currentY = startY + (50 - startY) * zp;

          if (planetRefs.earth.current) {
            planetRefs.earth.current.style.left = `${currentX}%`;
            planetRefs.earth.current.style.top = `${currentY}%`;
            planetRefs.earth.current.style.transform = `translate(-50%, -50%) scale(${scale})`;
            planetRefs.earth.current.style.opacity = String(
              Math.max(
                0,
                1 - smooth(invLerp(ZOOM_END - 0.05, ZOOM_END, progress)),
              ),
            );
            planetRefs.earth.current.style.zIndex = "100";
          }

          ["mercury", "venus", "mars"].forEach((name) => {
            if (planetRefs[name].current) {
              planetRefs[name].current.style.opacity = String(
                Math.max(0, 1 - zp * 4),
              );
            }
          });

          const bgP = smooth(invLerp(ZOOM_START, ZOOM_END, progress));
          if (atmoRef.current) atmoRef.current.style.opacity = String(bgP);

          // Phase 1: Fade clouds in (0 to 0.5)
          // Phase 2: Part clouds (0.5 to 1.0)
          let tp = 0;
          if (progress < ZOOM_END) {
            tp = invLerp(ZOOM_START + 0.05, ZOOM_END, progress) * 0.5;
          } else {
            tp = 0.5 + invLerp(REVEAL_START, REVEAL_END, progress) * 0.5;
          }

          transitionProgress.set(tp);

          if (cloudsRef.current) {
            cloudsRef.current.style.opacity = "1";
          }

          if (scheduleVisibleRef.current) {
            const schedAlpha = smooth(invLerp(0.55, 0.8, tp));
            scheduleVisibleRef.current.style.opacity = String(schedAlpha);
            scheduleVisibleRef.current.style.pointerEvents =
              schedAlpha > 0.5 ? "auto" : "none";
            scheduleVisibleRef.current.style.transform = `scale(${0.9 + schedAlpha * 0.1})`;
          }

          canvas.style.opacity = String(Math.max(0, 1 - zp));
          if (nebulaRef.current)
            nebulaRef.current.style.opacity = String(Math.max(0, 1 - zp * 2));

          if (rootRef.current) {
            rootRef.current.style.background = `linear-gradient(115deg, 
              rgba(98, 207, 244, ${bgP}), 
              rgba(44, 103, 242, ${bgP})
            ), #020818`;
          }
          ...
          if (scheduleVisibleRef.current) {
            scheduleVisibleRef.current.style.opacity = "0";
            scheduleVisibleRef.current.style.pointerEvents = "none";
          }
          transitionProgress.set(0);

          canvas.style.opacity = "1";
          if (nebulaRef.current) nebulaRef.current.style.opacity = "1";
          if (rootRef.current) rootRef.current.style.background = "#020818";

          if (planetRefs.earth.current) {
            const startX = isMobile ? 62 : 58;
            const startY = isMobile ? 38 : 40;
            planetRefs.earth.current.style.left = `${startX}%`;
            planetRefs.earth.current.style.top = `${startY}%`;
            planetRefs.earth.current.style.transform = `translate(-50%, -50%) scale(1)`;
            planetRefs.earth.current.style.zIndex = "5";
          }
          ["mercury", "venus", "mars"].forEach((name) => {
            if (planetRefs[name].current) {
              planetRefs[name].current.style.opacity = "1";
            }
          });
        }
      } catch (err) {
        console.warn("ThemeScene update error:", err);
      }
    }

    // Helper to hide overlays if already jumped
    const checkJumpedCleanup = (progress) => {
      if (jumpedRef.current && progress >= ZOOM_END) {
        if (atmoRef.current) atmoRef.current.style.opacity = "0";
        if (cloudsRef.current) cloudsRef.current.style.opacity = "0";
        transitionProgress.set(0);
        return true;
    ...
    return () => {
      ...
      // Ensure overlays are hidden on unmount or jump
      if (atmoRef.current) atmoRef.current.style.opacity = "0";
      if (cloudsRef.current) cloudsRef.current.style.opacity = "0";
      transitionProgress.set(0);
    };
    ...
    <section id="theme">
      <div className="theme-root" ref={rootRef}>
        <div id="space-section-react" ref={spaceSectionRef}>
          <div className="theme-sticky" ref={stickyRef}>
            ...
            <div className="sec-text" ref={secTextRef}>
              <p>
                We use the word <span className="highlight">Themes</span>{" "}
                instead of tracks to encourage creative freedom and
                interdisciplinary ideas.
              </p>
              <p>
                Your project does not need to fit perfectly into one category
                &mdash; themes are meant to guide inspiration, not limit
                innovation.
              </p>
              <p
                className="highlight"
            ...
            <Planets
              ...
              planetRef={planetRefs.venus}
              className="planet"
            />
            <Planets
              name="EARTH"
              textureUrl="/flat_earth.png"
            ...
            <Planets
              ...
              planetRef={planetRefs.mars}
              className="planet"
            />

            <div
              className="pulse-ring"
              ref={ringRefs.mercury}
              style={{
                width: 70,
                height: 70,
                left: "17%",
                top: "42%",
                borderColor: "rgba(200,160,130,.6)",
              }}
            />
            <div
              className="pulse-ring"
            ...
            <div
              ...
              style={{
                ...
                left: "80%",
                top: "53%",
                borderColor: "rgba(200,70,40,.6)",
              }}
            />

            <div className="theme-atmo" ref={atmoRef} />
            <div
              className="absolute inset-0 z-[115] opacity-0 pointer-events-none flex flex-col items-center justify-start overflow-y-auto pb-20 theme-section-schedule-container"
              ref={scheduleVisibleRef}
              style={{
                scrollbarWidth: "none",
            ...
            >
              <style
                dangerouslySetInnerHTML={{
                  __html: `
                .theme-section-schedule-container::-webkit-scrollbar { display: none; }
              `,
                }}
              />
            ...
            <div className="theme-clouds" ref={cloudsRef}>
              <Clouds isTransition transitionProgress={transitionProgress} />
            </div>
            ...
            <div className="track-panel pn-mercury" ref={panelRefs.mercury}>
              ...
              <div className="pn-desc">
                Innovations that improve wellness, care, and accessibility.
              </div>
            </div>
            ...
            <div className="track-panel pn-venus" ref={panelRefs.venus}>
              ...
              <div className="pn-div" />
              <div className="pn-desc">
                Tools that help people and organizations work smarter.
              </div>
            </div>
            ...
            <div className="track-panel pn-earth" ref={panelRefs.earth}>
              ...
              <div className="pn-div" />
              <div className="pn-desc">
                Tech solutions for a cleaner, greener future.
              </div>
            </div>
            ...
            <div className="track-panel pn-mars" ref={panelRefs.mars}>
              ...
              <div className="pn-div" />
              <div className="pn-desc">
                Creative, interactive, and impactful learning and play.
              </div>
            </div>
          </div>
        </div>
